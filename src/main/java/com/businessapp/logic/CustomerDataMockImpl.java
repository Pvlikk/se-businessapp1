...
class CustomerDataMockImpl implements CustomerDataIntf {
	...
	public void start() {
		...
		if( name.equals( "Kunden" ) ) {
			...
			Customer alfons = DS.newCustomer("Alfons", "Kipp");
			alfons.addContact("alfons284@gmail.com");
			alfons.setStatus(Customer.CustomerStatus.ACTIVE);
			eric.addContact( "e532@yahoo.com" );
			anja.addContact( "anja.schuhmann@benz.de" );
		} else {
			if( name.equals("Studenten")){
				DS.newCustomer( "Pavel", "Bodrov" ).addContact( "Grossweg 4/0, 79805 Aschaffenburg" );
				DS.newCustomer( "Murat", "Yukari" ).addContact( "Engelbert-Noack-Gasse 3, 16665 Parsberg" );
				DS.newCustomer( "Cihan", "Cavusoglu" ).addContact( "Starkplatz 8, 79663 Wolfratshausen" );
			}else {
				// Customer list 2
